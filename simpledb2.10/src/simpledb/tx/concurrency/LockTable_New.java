...
class LockTable_New {
   ...
   private Map<Block,Long> waitlist = new HashMap<Block,Long>();
    ...
    */
   public synchronized void sLock(Block blk, long timestamp) {
	   try {
	         long timestamp2 = System.currentTimeMillis();  
	         while (hasXlock(blk) && timestamp2< timestamp)
	        		wait();
	         if (hasXlock(blk))
    ...
    * @param blk a reference to the disk block
    */
   synchronized void xLock(Block blk, long timestamp) {
	      try {
	         long timestamp2 = System.currentTimeMillis();
	         while (hasOtherSLocks(blk) && timestamp2<timestamp)
	            wait();
   ...
   private Long getWaitVal(Block blk) {
	  Long ival = waitlist.get(blk);
	  return (long) ((ival == null) ? 0 : ival.intValue());
   }
}
